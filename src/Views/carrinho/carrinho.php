<div class="mt-8" id="carrinho-box">
    <?php if (isset($_SESSION['carrinho']) && count($_SESSION['carrinho']) > 0): ?>
        <ul class="space-y-4">
            <?php foreach ($_SESSION['carrinho'] as $key => $produto): ?>
                <li class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-sm text-gray-900"><?= $produto['nome'] ?></h3>
                        <dl class="mt-0.5 space-y-px text-[10px] text-gray-600">
                            <div>
                                <dt class="inline">Preço:</dt>
                                <dd class="inline">R$ <?= $produto['preco'] ?></dd>
                            </div>
                            <div>
                                <dt class="inline">Quantidade:</dt>
                                <dd class="inline"><?= $produto['quantidade'] ?></dd>
                            </div>
                        </dl>
                    </div>

                    <div class="flex flex-1 items-center justify-end gap-2">
                        <form action="" name="remove" method="GET">
                        ...
                        <button type="submit" class="text-gray-600 transition hover:text-red-600" onclick="removerItem('<?= $key ?>')">
        ...
        <div class="mt-8 flex justify-end border-t border-gray-100 pt-8">
            <div class="w-screen max-w-lg space-y-4">
                ...
                <div class="flex justify-end gap-2">
                    <a
                        href="<?= HOME ?>"
                        class="block rounded-sm bg-blue-700 px-5 py-3 text-sm text-gray-100 transition hover:bg-gray-600">
                        Continuar comprando
                    </a>
                    <a
                        href="#"
        ...
        <?php else: ?>
            ainda não possui nenhum item no carrinho.
    <?php endif; ?>
</div>